    ...
    @auth
        ...
        @php
            ...
            foreach (App\Models\InventoryCategory::all() as $category) {
                $inventoryNav->push([
                    'title' => title_case($category->name),
                    'url' => route('items.index', $category->slug),
                    'permission' => request()
                        ->user()
            ...
            $navigations = collect([
                ...
                [
                    ...
                    'childrens' => collect($inventoryNav),
                ],
                [
                    'title' => 'Services',
                ...
                [
                    ...
                    'childrens' => collect([
                        [
                            'title' => 'General',
                            'url' => route('general-audits'),
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('audits-general-view'),
                        ],
                        [
                            'title' => 'Service',
                            'url' => route('service-audits'),
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('audits-service-view'),
                        ],
                        [
                            'title' => 'Inventory',
                            'url' => route('inventory-audits'),
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('audits-inventory-view'),
                        ],
                    ]),
                ],
                [
                    ...
                    'permission' => request()
                        ->user()
                        ->hasAnyPermission(['configuration-general', 'configuration-data-import']),
                    ...
                    'childrens' => collect([
                        [
                            ...
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('configuration-general'),
                        ],
                        [
                            'title' => 'Data Import',
                            ...
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('configuration-data-import'),
                        ],
                    ]),
                ],
                [
                    ...
                    'childrens' => collect([
                        [
                            ...
                            'url' => route('inventory-ledger.index'),
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('report-inventory-ledger-view'),
                        ],
                        [
                            'title' => 'Dispensing',
                            'url' => route('dispensing.index'),
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('report-dispensing-view'),
                        ],
                        [
                            'title' => 'Cash Book',
                            ...
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('report-cash-view'),
                        ],
                        [
                            'title' => 'Patient Report',
                            'url' => route('patient-visit.index'),
                            'permission' => request()
                                ->user()
                                ->hasAnyPermission('report-patient-visit-view'),
                        ],
                    ]),
                ],
            ]);
                    ...
                    <ul class="nav pcoded-inner-navbar sidenav-inner">
                        @foreach ($navigations as $navigation)
                            ...
                            @if ($navigation['permission'] || $userRole == 'super-admin')
                                <li
                                    class="nav-item  {{ $HasChildrens ? 'pcoded-hasmenu' : '' }} {{ $HasChildrens ? ($navigation['childrens']->contains('url', $currentPath) ? 'active' : '') : ($navigation['url'] == $currentPath ? 'active' : '') }}">
                                    <a href="{{ $navigation['url'] }}" class="nav-link"><span class="pcoded-micon"><i
                                                class="{{ $navigation['icon'] }}"></i></span><span
                                            class="pcoded-mtext">{{ $navigation['title'] }}</span></a>
                                    @if ($HasChildrens)
                                        <ul class="pcoded-submenu">
                                            @foreach ($navigation['childrens'] as $nav)
                                                @if ($nav['permission'] || $userRole == 'super-admin')
                                                    <li class="{{ $nav['url'] == $currentPath ? 'active' : '' }}"><a
                                                            href="{{ $nav['url'] }}">{{ $nav['title'] }}</a></li>
                                                @endif
                                            @endforeach
                                        </ul>
                                    @endif
                        ...
                        <li>
                            <div class="dropdown drp-user">
                                ...
                                <div class="dropdown-menu dropdown-menu-right profile-notification">
                                    <div class="pro-head">
                                        <img src="{{ asset('image/patient-male.png') }}" class="img-radius"
                                            alt="User-Profile-Image">
                                        <span>{{ auth()->user()->name }}</span>
                                        <a href="javascript:void(0)" onclick="$('#logout').submit()" class="dud-logout"
                                            title="Logout">
                                            <i class="feather icon-log-out"></i>
                                        </a>
                                    </div>
                                    <ul class="pro-body">
                                        <li><a href="{{ route('my.profile') }}" class="dropdown-item"><i
                                        ...
                                        <div class="row align-items-center">
                                            <div class="col-md-12">
                                                <div class="page-header-title">
                                                    <h5 class="m-b-10 text-capitalize">
                                                        {{ strtoupper($segment_3 ?? ($segment_2 ?? $segment_1)) }}</h5>
                                                </div>
                                                <ul class="breadcrumb">
                                                    ...
                                                    @if ($segment_2)
                                                        <li class="breadcrumb-item"><a
                                                                href="{{ Route::has($segment_2 . '.index') ? route($segment_2 . '.index') : '#' }}">{{ $segment_2 }}</a>
                                                        </li>
                                                    @endif
                                                    @if ($segment_3)
                                                        <li class="breadcrumb-item"><a
